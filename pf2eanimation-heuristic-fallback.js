/**
 * PF2e Animation Heuristic Fallback Engine (v4.8 Hardened Production Build)
 * Feature Set: 
 *   - Deep Description Keyword Parsing (Infers damage types, schools, shields, and conditions)
 *   - PIXI Native Blend Mode Routing (Ensures cross-version hardware acceleration)
 *   - Isolated Teleportation Engine (Zero template-hang risk for blink/warp spells)
 *   - Persistent Crowd-Control Restraint Engine (Vines/Chains overlay with smooth scaling)
 *   - Robust PF2e Lifecycle Condition Unlinking (Native system item hooks prevent canvas leaks)
 */
...
// ============================================================
// 2. GLOBAL CACHING, DICTIONARIES & SAFE BLENDS
// ============================================================
const ASSET_CACHE = new Map();
const CONFIG_CACHE = new Map(); // Performance optimization optimization: caches parsed spell descriptions
let CURATED_SPELL_SET = null;
let activeSequences = 0;
const MAX_CONCURRENT = 4;

// Safe runtime lookup for PIXI Blend Modes across V11 and V12 architectures
const SAFE_ADD_BLEND = typeof PIXI !== 'undefined' ? (PIXI.BLEND_MODES?.ADD ?? 1) : 1;
...
const JB2A_DICT = {
    elements: {
        ...
        spirit:      { key: "divine",        color: "bluegold",  projectile: "guiding_bolt",    impact: "divine",    blend: true },  
...
function safeGetFile(preferredPath, fallbackPath) {
    ...
    try {
        if (typeof Sequencer !== 'undefined') {
            if (Sequencer.Database?.hasEntry(preferredPath) || Sequencer.hasFile?.(preferredPath)) {
...
// ============================================================
// 4. CORE PARSER & CACHED DESCRIPTION SCRAPER
// ============================================================
export function parseSpellToAnimation(spell) {
    const cacheKey = spell.uuid || spell.id;
    if (CONFIG_CACHE.has(cacheKey)) return CONFIG_CACHE.get(cacheKey);
    ...
    const configResult = { pattern, castRingPath, groundRingPath, schoolOverlay, primaryAsset, impactAsset, 
                          systemArea: system.area, intensity, inflictedConditions, isPersistentArea, useBlend, theme, traits };
    
    CONFIG_CACHE.set(cacheKey, configResult);
    return configResult;
}
...
export async function executeHeuristicAnimation(spell, token) {
    ...
    // Isolated AOE pattern tracking logic to prevent Teleportation from hanging
    ...
    const applyBlend = (effect, blend = config.useBlend) => {
        if (blend) effect.blendMode(SAFE_ADD_BLEND);
        return effect;
    };
    ...
    try {
        ...
        // --- Stage 2: School Overlay (Loops as a kinetic shield if Abjuration matches) ---
        if (config.schoolOverlay) {
            const effectBlock = seq.effect()
                .file(config.schoolOverlay)
                .atLocation(token)
                .scaleToObject(1.3 * config.intensity)
                .fadeIn(300);
            
            if (config.traits.includes("abjuration")) {
                effectBlock.duration(2500).fadeOut(600);
            } else {
                effectBlock.duration(800);
            }
        }
        ...
        switch (config.pattern) {
            case "TELEPORTATION": {
                // Fixed: Stripped out of template listener dependency loops entirely
                let destination = targets[0] || null;
            ...
            case "RESTRAINT": {
                ...
                for (let target of actualTargets) {
                    // Fixed: Replaced fragile custom origin loops with Sequencer's robust target attachment hooks
                    seq.effect()
                        .file(config.groundRingPath)
                        .attachTo(target)
                        .belowTokens()
                    ...
                    if (isLongDuration) {
                        binding.persist().duration(300000); // 5-minute security hard ceiling
                    } else {
...
Hooks.on("createChatMessage", async (message, options, userId) => {
    ...
    if (token) {
        console.log(`PF2e Heuristic | v4.8 animating: ${spell.name}`);
        executeHeuristicAnimation(spell, token);
    }
});

// ============================================================
// 7. HARDEED PF2E NATIVE CONDITION LIFECYCLE UNLINKING HOOKS
// ============================================================
// Fixed: Intercepting system condition document mutations directly via explicit creation/deletion channels
const CC_SLUGS = ["immobilized", "restrained", "paralyzed"];

Hooks.on("createItem", (item, options, userId) => {
    if (item.type !== "condition" || !CC_SLUGS.includes(item.slug)) return;
    console.debug(`PF2e Heuristic | Mobility condition [${item.slug}] applied to ${item.actor?.name}.`);
});
...
Hooks.on("deleteItem", (item, options, userId) => {
    ...
    for (let token of activeTokens) {
        // Clean target dismissal using explicit token name registers
        if (typeof Sequencer !== 'undefined') {
            Sequencer.EffectManager.endEffects({ name: `restraint-ground-${token.id}` });
            Sequencer.EffectManager.endEffects({ name: `restraint-binding-${token.id}` });
        }
    }
});